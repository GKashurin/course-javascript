...
function isAllTrue(array, fn) {
  if (typeof fn !== 'function') {
    // проверяем, чтобы fn была функцией
    throw new Error('fn is not a function');
  }
  if (!Array.isArray(array) || !array.length) {
    //Array.isArray специальный встроенный меиод, который определяет, является это массивом или нет или пустой массив
    throw new Error('empty array');
  }
  for (const el of array) {
    if (!fn(el)) {
      //если хоть одна итерция хоть для одного элемента вернула false, вся функция вернет false
      return false;
    }
  }
  return true;
